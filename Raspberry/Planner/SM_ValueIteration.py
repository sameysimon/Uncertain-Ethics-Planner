...
class Solver:
    ...
    def solve(solver, problem:SM_MDP) -> dict:
        ...
        converged=False
        i=0
        E = [0]*len(problem.states)
        while not converged:
            E_ = copy.deepcopy(E)
            for s in problem.states:
                actions = problem.getActions(s)
                if len(actions)!=0:
                    l = solver.actionEstimates(s,problem,actions,E)
        ...
        for s in problem.states:
            ...
            l = solver.actionEstimates(s,problem, actions, E)
            if len(actions)!=0:
                pi[s.id] = actions[solver.idxPreferred(l, problem.Theory)]
        return pi

    def actionEstimates(solver, state, problem, actions, E):
        l = []
        for a in actions:
    ...
    def preferred(solver, l, theory):
        return l[solver.idxPreferred(l, theory)]
